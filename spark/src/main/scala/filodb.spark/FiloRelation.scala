      ...
      if (filters.length == 1) {
        ...
        filters match {
          case Seq(EqualTo(_, filterValue)) =>
            ...
            //Some(SegmentRange(equalVal,equalVal).basedOn(projection)).headOption
          Option(SegmentRange(equalVal, equalVal).basedOn(projection).asInstanceOf[SegmentRange[projection.SK]])
          case other: Any => None
        }
